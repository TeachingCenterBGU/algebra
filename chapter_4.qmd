...
::: proof
*Proof.* לפי טענה `\ref{prop:elementary}`{=latex} מתקיים $EA=S(A)$. לכן $$.\det(EA)=\det(S(A))=\begin{cases}
...
::: proof
*Proof.* לפי `\ref{prop:det-product}`{=latex} מתקיים $$.\det(A)\det(A^{-1})=\det(AA^{-1})=\det(I_n)=1$$ נחלק ב- $\det(A)$ ונקבל $$.\det(A^{-1})=\frac{1}{\det(A)}$$ ◻
:::
