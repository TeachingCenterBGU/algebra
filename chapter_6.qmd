...
בהינתן מ\"ו וקטורי $V$, ראינו את ההגדרה של תת-מרחב $W\subseteq V$ וגם את הקשר בין המימדים במסקנה [8.95](chapter_5.qmd#cor-subspace-dim). בפרק זה נרצה להבין מהן הפעולות שאפשר לבצע על שני תת-מרחבים (או יותר), כאשר המטרה היא לקבל תת-מרחבים נוספים.
...
::: proof
 לפי טענה [8.14](chapter_5.qmd#prp-subspace), צריך להראות כי $0_V\in W_1\cap W_2$ ובנוסף שיש סגירות לחיבור וכפל בסקלר.
...
::: proof
לפי טענה [8.14](chapter_5.qmd#prp-subspace), כדי להוכיח כי $W_1+W_2$ הוא תת-מרחב, יש להראות כי $0_V\in W_1+W_2$ ובנוסף שיש סגירות לחיבור וכפל בסקלר.
...
השתמשנו בהצבה $p=r+t$. נלמד בהמשך דרך כללית יותר למצוא בסיסים לחיתוך וסכום של תת-מרחבים. השוויון האחרון נכון לפי משפט [8.68](chapter_5.qmd#thm-invertible2), למשל כי
...
::: proof
זו מסקנה ישירה משילוב של מסקנה [8.95](chapter_5.qmd#cor-subspace-dim) וההכלות הבאות:
...
::: {.callout-note collapse="true" title="פתרון"}
נניח כי $\dim(W_1+W_2)=\dim(W_2)$. מאחר שמתקיים $W_2\subseteq W_1+W_2$, שוויון מימדים גורר $W_2=W_1+W_2$ לפי מסקנה [8.95](chapter_5.qmd#cor-subspace-dim). אבל גם מתקיים $W_1\subseteq W_1+W_2$, ולכן $W_1\subseteq W_2$.
:::
...
הקבוצה $B_1=\Set{w_1,...,w_m,w_{k+1},...,w_m}$ היא בסיס, ולכן הסקלרים נקבעים ביחידות לפי טענה [8.63](chapter_5.qmd#prp-unique-scalars). מכאן $$.\alpha_i=\begin{cases}
...
לפי טענה [6.46](chapter_3.qmd#prp-sym-antisym) מתקיים: $$\begin{aligned}
...
א. לפי מסקנה [8.95](chapter_5.qmd#cor-subspace-dim), מתקיים $$,W_1+W_2=\mathbb{C}^4 \iff \dim(W_1+W_2)=4$$ ולפי נוסחת המימדים נובע כי $$.\dim(W_1+W_2)=4 \iff 2+2-\dim(W_1\cap W_2)=4 \iff \dim(W_1\cap W_2)=0$$
...
בתת-פרק [8.5.2](chapter_5.qmd#sub-extraction) ראינו שניתן למצוא בסיס למרחב העמודות ע\"י דירוג קנוני, לפיו העמודות בהן יש איברים מובילים מתאימות לעמודות המקוריות שמהוות יחד בסיס למרחב העמודות (אין לקחת את העמודות החדשות, כי הדירוג משנה את מרחב העמודות). במקרה זה, מרחב העמודות הוא $W_1+W_2$.
...
קיבלנו שהקבוצה $\Set{\begin{pmatrix}-1 \\ -1 \\ 1 \\ 1\end{pmatrix}}$ היא בסיס ל- $W_1\cap W_2$, ולכן $\dim(W_1\cap W_2)=1$. בנוסף, $\dim(W_2)=2$ כי הקבוצה הפורשת אותו היא בת\"ל (הוקטורים אינם קו-לינאריים) ולכן בסיס. באופן דומה, $\dim(W_1)=4-\mathop{\mathrm{rank}}(A)=4-2=2$ לפי משפט [8.99](chapter_5.qmd#thm-rank-nullity) (משפט הדרגה). לכן, לפי נוסחת המימדים נובע כי $$.\dim(W_1+W_2)=2+2-1=3$$ כדי למצוא בסיס ל- $W_1+W_2$ מספיק להוסיף לקבוצה הפורשת את $W_2$ כל וקטור ב- $W_1$ שאינו בחיתוך, כי זה בדיוק אומר שהוא לא נפרש ע\"י וקטורי הבסיס של $W_2$. כך נקבל קבוצה בת\"ל ב- $W_1+W_2$ שגודלה שווה למימדו, ולכן היא בהכרח בסיס.
