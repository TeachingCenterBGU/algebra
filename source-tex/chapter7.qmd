...
::: solution
**פתרון 1.1**. ראינו שמימד החיתוך הוא $n$, כמספר המשבצות על האלכסון הראשי. בשביל $\dim(W_1)$ צריך להוסיף את מספר המשבצות מעל האלכסון הראשי. דרך אחת לחשב אותו הוא ע\"י הנוסחה לסכום סדרה חשבונית, אבל נשתמש בדרך אחרת. משיקולי סימטריה, מספר זה שווה למספר המשבצות מתחת לאלכסון הראשי. אם נסמן מספר זה ב- $k$, הרי שניתן לספור את כלל המשבצות בשתי דרכים שונות ולקבל $$.2k+n=n^2 \implies k=\frac{n^2-n}{2}$$ מכאן נובע כי $$.\dim(W_1)=\dim(W_2)=k+n=\frac{n^2-n}{2}+n=\frac{n(n+1)}{2}$$
:::
...
::: proof
*Proof.* לפי טענה `\ref{prop:subspace}`{=latex}, צריך להראות כי $0_V\in W_1\cap W_2$ ובנוסף שיש סגירות לחיבור וכפל בסקלר.
...
::: solution
**פתרון 1.3**. בכיוון אחד, נניח כי $W_1\subseteq W_2$ (המקרה של $W_2\subseteq W_1$ דומה משיקולי סימטריה). אז מתקיים $W_1\cup W_2=W_2$, ובפרט זהו תת-מרחב.
...
::: definition
 `\adddeftolist{סכום של תת-מרחבים}`{=latex} יהי $V$ מ\"ו מעל $\mathbb{F}$, ויהיו $W_1,W_2\subseteq V$ תת-מרחבים. אז הסכום $W_1+W_2$ מוגדר ע\"י $$.W_1+W_2=\Set{w_1+w_2|w_1\in W_1,w_2\in W_2}$$
:::
...
<figure id="fig:IntersectionSum">
<img src="images/IntersectionSum.png" style="width:60.0%" />
...
::: proof
*Proof.* לפי טענה `\ref{prop:subspace}`{=latex}, כדי להוכיח כי $W_1+W_2$ הוא תת-מרחב, יש להראות כי $0_V\in W_1+W_2$ ובנוסף שיש סגירות לחיבור וכפל בסקלר.
...
2.  נסמן ב- $\mathbb{R}^3$ את תת-המרחבים הבאים: $$\begin{aligned}
    W_1&=\mathop{\mathrm{Span}}((1,0,0),(1,1,1)) \\
    W_2&=\mathop{\mathrm{Span}}((0,1,0),(1,1,1)))
    \end{aligned}$$ לא קשה לבדוק כי $W_1\neq W_2$, ולכן באופן אינטואיטיבי $W_1+W_2$ אמור להיות ממימד גבוה יותר מ- $W_1$ וגם מ- $W_2$ (במקרה זה הם שווי-מימד). אז אמור להתקיים $$,\dim(W_1+W_2)=3$$ כי אין אפשרות למימד גבוה מזה ב- $\mathbb{R}^3$. בהמשך נראה טיעון פורמלי מסוג זה, המסתמך על שיקולי מימד, אבל בינתיים נראה כי $$W_1+W_2=\mathbb{R}^3$$ לפי הגדרת הסכום. נשים לב כי $(1,1,1)\in W_1\cap W_2$ ונשתמש בעובדה זו כדי להציג את הסכום בצורה פשוטה יותר, במקום לחזור על $(1,1,1)$. $$\begin{aligned}
    W_1+W_2&=\Set{q(1,0,0)+r(1,1,1)+s(0,1,0)+t(1,1,1)|q,r,s,t\in\mathbb{R}} \\
    &=\Set{q(1,0,0)+s(0,1,0)+(r+t)(1,1,1)|q,r,s,t\in\mathbb{R}} \\
    &=\Set{q(1,0,0)+s(0,1,0)+p(1,1,1)|p,q,s\in\mathbb{R}} \\
    &=\mathop{\mathrm{Span}}((1,0,0),(0,1,0),(1,1,1))=\mathbb{R}^3
    \end{aligned}$$ השתמשנו בהצבה $p=r+t$. נלמד בהמשך דרך כללית יותר למצוא בסיסים לחיתוך וסכום של תת-מרחבים. השוויון האחרון נכון לפי משפט `\ref{thm:invertible2}`{=latex}, למשל כי $$.\det\begin{pmatrix}
    1 & 0 & 0 \\
    0 & 1 & 0 \\
    1 & 1 & 1
    \end{pmatrix}=1^3=1\neq 0$$
...
::: solution
**פתרון 1.4**. מתקיים $$\begin{aligned}
...
::: solution
**פתרון 1.5**. הטענה אינה נכונה. נפריך אותה ע\"י הדוגמה הנגדית הבאה:
...
*יהיו $V$ מ\"ו מעל $\mathbb{F}$ ו- $W_1,W_2\subseteq V$ תת-מרחבים. אז מתקיים: $$\begin{aligned}
...
::: proof
*Proof.* זו מסקנה ישירה משילוב של מסקנה `\ref{cor:subspace-dim}`{=latex} וההכלות הבאות:
...
::: solution
**פתרון 1.6**. נניח כי $\dim(W_1+W_2)=\dim(W_2)$. מאחר שמתקיים $W_2\subseteq W_1+W_2$, שוויון מימדים גורר $W_2=W_1+W_2$ לפי מסקנה `\ref{cor:subspace-dim}`{=latex}. אבל גם מתקיים $W_1\subseteq W_1+W_2$, ולכן $W_1\subseteq W_2$.
:::

הנוסחה במשפט הבא נקראת נוסחת המימדים, והיא מבטאת קשר יותר מדויק בין המימדים של שני תת-מרחבים, חיתוכם וסכומם.

::: theorem
*`\label{thm:dim-formula}`{=latex} `\addtheoremtolist{נוסחת המימדים}`{=latex} יהי $V$ מ\"ו נוצר סופית מעל $\mathbb{F}$, ויהיו $W_1,W_2\subseteq V$ תת-מרחבים. אז מתקיים $$.\dim(W_1+W_2)=\dim(W_1)+\dim(W_2)-\dim(W_1\cap W_2)$$*
:::
...
w_1  & \cdots & w_m  & -u_1  & \cdots & -u_k \\
\mid &        & \mid & \mid &        & \mid
\end{pmatrix}$$ זו לא בדיוק המטריצה הקודמת, ולכאורה דרוש דירוג קינוני חדש. אך יש דרך קיצור, כי במשוואה `\ref{eqn:intersection2}`{=latex} ניתן להציב $\gamma_1=-\beta_1,...,\gamma_k=-\beta_k$, ואז המטריצה המתאימה לממ\"ל החדשה בנעלמים $\alpha_1,...,\alpha_m,\beta_1,....,\beta_k$ היא המטריצה שכבר דירגנו: $$\begin{pmatrix}
\mid &        & \mid & \mid &        & \mid \\
w_1  & \cdots & w_m  & u_1  & \cdots & u_k \\
\mid &        & \mid & \mid &        & \mid
...
1 \\
-1   
\end{pmatrix}$$ אז בהכרח קיים $t\in\mathbb{R}$ כך ש- $$.\alpha_1=\alpha_2=-t,\beta_1=t,\beta_2=0$$ מכאן: $$W_1\cap W_2=\Set{-t\begin{pmatrix}
1 \\
1 \\
1 \\
1   
...
::: solution
**פתרון 1.8**. דרך אחת לפתור את התרגיל היא למצוא בסיס ל- $\mathop{\mathrm{N}}(A)$. בבסיס זה יש שני וקטורים, ולכן יחד עם הוקטורים בקבוצה הפורשת את $W_2$, נקבל מטריצה של $4$ עמודות. אין שום בעיה עם דרך זו, אך היא דורשת שני דירוגים וכדאי להכיר גישה קצת שונה.
...
`\noindent`{=latex}**הגדרה 9.1.**

יהי $V$ מ\"ו מעל $\mathbb{F}$, ויהיו $W_1,W_2\subseteq V$ תת-מרחבים. אז החיתוך $W_1\cap W_2$ הוא קבוצת כל הוקטורים השייכים גם ל- $W_1$ וגם ל- $W_2$, כלומר $$.W_1\cap W_2=\Set{v\in V|v\in W_1,v\in W_2}$$ `\bigskip`{=latex} `\bigskip`{=latex}

`\noindent`{=latex}**טענה 9.2.**

יהי $V$ מ\"ו מעל $\mathbb{F}$, ויהיו $W_1,W_2\subseteq V$ תת-מרחבים. אז $W_1\cap W_2$ הוא תת-מרחב של $V$. `\bigskip`{=latex} `\bigskip`{=latex}

`\noindent`{=latex}**הגדרה 9.3.**

 יהי $V$ מ\"ו מעל $\mathbb{F}$, ויהיו $W_1,W_2\subseteq V$ תת-מרחבים. אז הסכום $W_1+W_2$ מוגדר ע\"י $$.W_1+W_2=\Set{w_1+w_2|w_1\in W_1,w_2\in W_2}$$ `\bigskip`{=latex} `\bigskip`{=latex}

`\noindent`{=latex}**טענה 9.4.**

יהי $V$ מ\"ו מעל $\mathbb{F}$, ויהיו $W_1,W_2\subseteq V$ תת-מרחבים. אז $W_1+W_2$ הוא תת-מרחב של $V$, המקיים $W_1\subseteq W_1+W_2$ וגם $W_2\subseteq W_1+W_2$. `\bigskip`{=latex} `\bigskip`{=latex}

`\noindent`{=latex}**מסקנה 9.5.**

יהיו $V$ מ\"ו מעל $\mathbb{F}$ ו- $W_1,W_2\subseteq V$ תת-מרחבים. אז מתקיים: $$\begin{aligned}
\dim(W_1\cap W_2)\leq \dim(W_1)\leq\dim(W_1+W_2)\leq\dim(V)
\end{aligned}$$ ובאופן דומה: $$\dim(W_1\cap W_2)\leq \dim(W_2)\leq\dim(W_1+W_2)\leq\dim(V)$$ `\bigskip`{=latex} `\bigskip`{=latex}

`\noindent`{=latex}**משפט 9.6.**

יהי $V$ מ\"ו נוצר סופית מעל $\mathbb{F}$, ויהיו $W_1,W_2\subseteq V$ תת-מרחבים. אז מתקיים $$.\dim(W_1+W_2)=\dim(W_1)+\dim(W_2)-\dim(W_1\cap W_2)$$ `\bigskip`{=latex} `\bigskip`{=latex}
