...
\documentclass[oneside,american,hebrew]{book}
...
\makeatletter
\renewcommand{\@chapapp}{\iflanguage{hebrew}{פרק}{Chapter}}
\makeatother
...
\newtheoremstyle{hebrewcorollary}
{3pt} % Space above
{3pt} % Space below
{} % Indent amount
{} % Body font (normal shape, no color here)
...
{:} % Punctuation after theorem head
{ } % Space after theorem head
{\textcolor{green}{\thmname{#1}\thmnote{ (#3)}}} % Theorem head specification

\theoremstyle{hebrewcorollary}
\newtheorem{oldcor}{מסקנה}

\NewEnviron{cor}{%
	{\color{green}\begin{oldcor}\BODY\end{olddef}}%
}
...
\begin{remark} לפעמים גם \L{$0$} נחשב מספר טבעי, אבל לא בקורס שלנו
וזה לא באמת חשוב. מבחינה היסטורית ופילוסופית, \L{$0$} הוא מספר מוזר
ומיוחד כי לא רואים אותו בטבע. הוא מתאר את מה שאינו.
\end{remark}


\begin{hdef} קבוצה היא אוסף כלשהו של איברים )לא בהכרח מספרים( ללא
חשיבות לסדר הופעתם.
\end{hdef}
...
\begin{example} משפחת לוי כוללת את דרור )האב(, אורית )האם(
ואיתי )הבן(. נקרא לקבוצת אנשים זו \L{$L$} ונתאר אותה באופן מפורש
תוך שימוש בלועזית )אפשר גם בעברית(:\L{%
\[
...
\begin{self} האם מתקיים \L{$100^{100}\in\mathbb{N}$}? \L{$100^{-100}\in\mathbb{N}$}?
...
בה כל מספר מהצורה \L{$n\in\mathbb{N}$}. הפעם הכתיבה היא כדלקמן:\L{%
\[
...
\L{$n\in\mathbb{Z}$}. כך מקבלים את קבוצת המספרים הרציונליים:\L{%
\[
...
זה מביא אותנו לקבוצת המספרים הממשיים \L{$\mathbb{R}$}, שהיא הקבוצה
העיקרית שנתמקד בה בקורס. קבוצה זו מכילה את קבוצת המספרים הרציונליים
...
ואז החוקיות של סדרת הספרות העשרוניות עלולה להיות מסובכת מאוד. למשל:\L{%
\begin{eqnarray*}
...
\begin{hdef} עבור \L{$z=x+yi$} מרוכב עם \L{$x,y\in\mathbb{R}$}
...
\begin{self} לכל \L{$z\in\mathbb{C}$} הראו כי המספר הצמוד
...
\begin{hdef} נאמר ש-\L{$A$} מוכלת ב-\L{$B$} ונסמן \L{$A\subseteq B$}
...
\begin{example} עבור \L{$A=\{1,2\},\,B=\{1,2,3\}$} מתקיים \L{$A\subseteq B$}
כי גם \L{$1\in B$} וגם \L{$2\in B$}. אבל \L{$B\nsubseteq A$} כי
\L{$3\in B$} אך \L{$3\notin A$}.
\end{example}


\begin{self} נגדיר \L{$A=\{1,3\},\,B=\{-1,1,2,3\},\,C=\{-1,2\}$}.
מצאו את כל זוגות הקבוצות שמקיימות קשר של הכלה.
\end{self}


\begin{hdef} נאמר ששתי קבוצות \L{$A,B$} הן שוות ונסמן \L{$A=B$}
אם יש בהן בדיוק אותם האיברים, כלומר מתקיים \L{$x\in A$} אם ורק אם
\L{$x\in B$}.
\end{hdef}


\begin{remark} כדי להוכיח כי \L{$A=B$}, הדרך המקובלת היא להוכיח כי
\L{$A\subseteq B$} וגם \L{$B\subseteq A$}. דרך הוכחה זו נקראת הכלה
דו-צדדית.
\end{remark}


\begin{remark} הגדרנו את קבוצת המספרים הזוגיים החיוביים בשתי דרכים
...
\begin{prop} \L{$\mathbb{N\subseteq\mathbb{Z}\subseteq\mathbb{Q}\subseteq\mathbb{R}\subseteq\mathbb{\mathbb{C}}}$}
וכל הקבוצות שונות זו מזו.
\end{prop}


\begin{hproof} ברור כי \L{$\mathbb{N}\subseteq\mathbb{Z}$} לפי הגדרת
...
\begin{hdef} בהינתן שתי קבוצות \L{$A,B$} נגדיר את הקבוצות הבאות:
...
\begin{remark} במקרה של איחוד זה לא משנה אם האיבר שייך רק לקבוצה אחת
...
\begin{self} חשבו את \L{$A\cap B,\,A\cup B$} עבור \L{$A=\{-1,0,1\},\,B=\{-2,0,1,2\}$}.
\end{self}


\begin{prop} לכל שתי קבוצות \L{$A,B$} מתקיים\L{%
\[
...
\begin{hproof} יש כאן שתי הכלות. נוכיח תחילה כי \L{$A\cap B\subseteq A$}:
...
\begin{remark} באותו אופן )או משיקולי סימטריה( גם מתקיים \L{%
\[
...
\begin{example} ניקח \L{$z_{1}=1+i,\,z_{2}=2+3i$}. כאן נקבל:
\par
...
\begin{self} חשבו את ארבע הפעולות עבור \L{$z_{1}=1+2i,\,z_{2}=3-4i$}.
\end{self}
...
\begin{prop} לכל \L{$z_{1},z_{2},z_{3}\in\mathbb{C}$} מתקיים:
\par
...
\begin{hproof} ישירות מן ההגדרות של חיבור וכפל תוך שימוש בחוקים המוכרים
...
\begin{self} הוכיחו את חוק החילוף לחיבור של מספרים מרוכבים.
\end{self}
...
\begin{prop} לכל \L{$z_{1},z_{2}\in\mathbb{C}$} מתקיים:
\par
...
\begin{hproof} נסמן \L{$z_{1}=a+bi,\,z_{2}=c+di$}. נחשב:
\par
...
\begin{prop} לכל \L{$z\in\mathbb{C}$} מתקיים:
\par
...
\begin{hproof} נסמן \L{$z=x+yi$} כאשר \L{$\text{{Re}}(z)=x,\,\text{{Im}}(z)=y$}.
נחשב:
\par
...
\begin{remark} \L{$r$} נקרא הערך המוחלט של \L{$z$} ומתקיים \L{$r=\sqrt{z\overline{z}}$}.
...
\begin{example} בהינתן \L{$r=2,\,\theta=\frac{\pi}{4}$} נוכל לחשב
...
\begin{remark} הזווית לא מוגדרת עבור \L{$z=0$}. זהו מקרה יוצא דופן
אך פשוט, כך שגם אין צורך בקוארדינטות פולריות בשבילו.
\end{remark}


\begin{example} נחשב את הקוארדינטות הפולריות של \L{$z=-2+2i$}. נקבל
...
\begin{hdef} נסמן \L{$e^{i\theta}=\cos\theta+i\sin\theta$}.
\end{hdef}
...
\begin{theorem}: לכל \L{$\theta\in\mathbb{R},\,n\in\mathbb{Z}$}
...
\begin{prop} לכל \L{$\alpha,\beta\in\mathbb{R}$} מתקיים \L{$e^{i\alpha}e^{i\beta}=e^{i(\alpha+\beta)}$}.
\end{prop}


\begin{self}: השתמשו בטענה כדי להוכיח את המשפט במקרה \L{$n=2$}.
\end{self}


\begin{example} נחשב את \L{$(1+\sqrt{3}i)^{100}$} בעזרת משפט דה-מואבר
...
\begin{self} חשבו את \L{$(1+\sqrt{3}i)^{10}$}.
\end{self}
...
\begin{hproof} זו משוואה ממעלה \L{$n$} ולכן יש לה לכל היותר \L{$n$}
...
\begin{remark} שימו לב לשימוש במחזוריות של סינוס וקוסינוס. למעשה, \L{$z_{k}$}
...
\begin{self} במקרה של \L{$n=2$}, הראו כי שני השורשים מקיימים
\L{$z_{1}=-z_{0}$}. זה נובע מכך ששינוי סימן מתאים לסיבוב ב-\L{$\pi$}
)\L{$180^{\circ}$}( נגד כיוון השעון, כי \L{$e^{i\pi}=-1$}.
\end{self}


\begin{remark} עבור \L{$w=0$} יש שורש יחיד, שהוא \L{$z=0$}. זהו מקרה
מיוחד שבו השורשים מתלכדים ומתקבל שורש יחיד.
\end{remark}


\begin{example} נפתור את המשוואה \L{$z^{4}=1-i$}. ראשית נחשב קוארדינטות
...
{\beginL 1\endL}( פתרו את המשוואות הלינאריות הבאות:
...
{\beginL 2\endL}( נניח כי \L{$z=re^{i\theta}$} נתון בהצגה פולרית.
הראו כי\L{%
\[
.\begin{cases}
...
{\beginL 3\endL}( חשבו את החזקות הבאות:
...
{\beginL 4\endL}( מצאו את כל השורשים המרוכבים של המשוואות הבאות:
\par
